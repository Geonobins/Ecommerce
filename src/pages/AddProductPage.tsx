...
const AddProductPage = () => {
    ...
    const handleFormSubmit = async () => {
        ...
        if (!validateInput(title, description, category, subcategory)  || price < 0 || (images.length === 0 && existingImages.length === 0)) {
            if (price<0){
                setNotificationMsg("Invalid Price");
            }
            else{
                setNotificationMsg("All fields are required ")
            }
            setTimeout(() => {
    ...
    return (
        <div>
            <div className="flex-1 flex-col ">
                <Navbar />
                {notificationMsg && <Notification msg={notificationMsg} status="danger"/>}
